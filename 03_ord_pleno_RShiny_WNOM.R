...
tryCatch({
  #orden_votantes_t <- read_csv("ideological-scaling-files/03_orden_votantes_t.csv", show_col_types = FALSE)
  orden_votantes_t <- readRDS("ideological-scaling-files/03_orden_votantes_t.rds")
}, error = function(e) {
  stop("Error loading ideological-scaling-files/03_orden_votantes_t. Make sure the file exists in the app directory. Original error: ", e$message)
})
