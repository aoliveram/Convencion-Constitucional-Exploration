...
OUTPUT_PATH = os.path.join(BASE_DIR, "comision-2", "indicaciones-api-extracted", "indications_report_1_part5_6.json")
...
def extract_from_pdf_chunk(model, pdf_path, members_str):
    ...
    2. Ideally, group by ARTICLE. 
       ...
       - Extract the FULL text of Column 1 as "base_text".
...
def main():
    print("--- Extracting Report 1 Structure (Retry Parts 5 & 6) ---")
    ...
    # Using Pro 3 as requested
    model = genai.GenerativeModel('gemini-3-pro-preview')
    
    members = load_json(MEMBERS_PATH)
    members_str = ", ".join(members) 
    
    # Explicitly list file 5 and 6
    target_files = [
        os.path.join(PDF_DIR, "C2_COMPLEX_informe-reemplazo-1-03-02_5.pdf"),
        os.path.join(PDF_DIR, "C2_COMPLEX_informe-reemplazo-1-03-02_6.pdf")
    ]
    ...
    print(f"\nSaved retry results ({len(extracted_data)} blocks) to {OUTPUT_PATH}")
